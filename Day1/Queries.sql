...
ALTER TABLE customer ADD active BIT; -- Add a boolean "active" column (use BIT for SQL Server)

-- Step 5: Drop the Newly Added Column
ALTER TABLE customer DROP COLUMN active; -- Remove the "active" column

-- Step 6: Rename Columns
EXEC sp_rename 'customer.email', 'email_address', 'COLUMN'; -- Rename column in SQL Server
...
SELECT first_name, last_name FROM customer WHERE first_name LIKE '%EN%'; -- ILIKE (case-insensitive)  
